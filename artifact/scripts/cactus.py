...
def cum_sum(tool_name: str):
    ...
    q = (
        ...
        .filter(pl.col(tool_name).is_not_null())
        .cast({tool_name: pl.Int32})
        .filter(pl.col(tool_name) > 0)
...
speedups_lazy = (
    withbin
    .filter(pl.col("sweap-noacc").is_not_null())
    .filter(pl.col("sweap-nobin").is_not_null())
    .cast({x: pl.Int32 for x in ("sweap-noacc", "sweap-nobin")})
    .filter(pl.col("sweap-noacc") > 0).filter(pl.col("sweap-noacc") < TIMEOUT)
    .filter(pl.col("sweap-nobin") > 0).filter(pl.col("sweap-nobin") < TIMEOUT)
    .with_columns(
