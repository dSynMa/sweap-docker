...
def update_stats(result: int | None, tool: str, bench: str):
    if result is None:
        return
    if 0 < result < timeout:
...
def fmt_result(x: int|None, real: bool=False):
    if x is None:
        return ""
    if x == 0:
        return r"\ERROR"
...
def do_latex_body(benchs, source):
    for b, is_realizable in benchs.items():
        ...
        r = {x: fmt_result(results[b].get(x), False) for x in latex_order}

        # Temos' unrealizability verdicts cannot be trusted
        if not is_realizable and 0 < (results[b].get("temos") or 0) < timeout:
            r["temos"] = r"\ERR"
        ...
        positive_results = {
            ...
            if tool in syn_tools and (results[b].get(tool) or 0) > 1}
...
with open(out_dir / LTL_LATEX, "w") as latex:
    ...
    for b in ltl_benchs:
        ...
        try:
            ...
            if not 0 < (results[b].get(best) or 0) < timeout:
                best = None
        except TypeError:
            best = ""
...
for best, uniq, which_tools in ((syn_best, syn_uniq, syn_tools), (r11y_best, r11y_uniq, r11y_tools)):
    for b in infinite_benchs:
        ...
        good_times = {
            ...
            if tool in which_tools and t is not None and 2 < t < timeout}
        if (good_times):
...
with open(out_dir / SUMM_LATEX, "w") as latex:
    ...
    all_solved = {
        t: sum(
            int(0 < (results[b].get(t) or 0) < timeout)
...
ltl_times = {
    t: {b: results[b].get(t, 0)
        for b in results 
        if b in ltl_benchs and 0 < (results[b].get(t) or 0) < timeout}
    for t in tools if "sweap" in t}

# In how many benchmarks the lazy approach was the best
lazy_best = sum(
    ltl_times["sweap-noacc"][b] < (ltl_times["sweap"].get(b) or 0)
    for b in ltl_times["sweap-noacc"])

no_refinements = sum(
    int(refinements[b].get("sweap", (0,0,0))[2]) == 0
    and int(refinements[b].get("sweap", (0,0,0,0))[3]) == 0
    for b in ltl_benchs)
